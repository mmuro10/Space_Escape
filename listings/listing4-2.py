...
def get_floor_type():
    ...
    global top_left_x, top_left_y, wall_transparency_fram
    ...
    for y in range(roomroom_height -2):
    ...
    if current_room % MAP_WIDTH != 1: #If room is not on left of map
        ...
        #if room on the left has right exi, add left exit in this room
        if room_to_left[4]:
    ...
    if current <= MAP_SIZE - MAP_WIDTH: #if room is not on bottome row
        room_below = GAME_MAP[ccurrent_room + MAP_WIDTH]
        #If room below has a top exit, add exit at bottom of this one
        if room_below[3]:
            room_map[room_height-1][middle_coumn] = floor_type
            room_map[room_height-1][middle_coumn + 1] = floor_type
            room_map[room_height-1][middle_coumn - 1] = floor_type